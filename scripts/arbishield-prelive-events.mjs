  ...
  try {
    ...
    for (const t of list) {
      ...
      if (String(t.type || "") === "exchange_commission") {
        commissionCharged += Math.abs(nCents(t.amount_cents));
        hasTx = true;
        continue;
      }
...
async function creditWalletForSettlement(row, outcome, now) {
  ...
  if (!wonArbi && !isVoid) {
    ...
    // INSERT: Comissão Exchange 4,5% do lucro (não altera a lógica da dedução)
    ...
    if (commissionDue > 0) {
      ...
      // saldos já refletidos no patch (pós-dedução)
      const curDemo =
        patch.demo_balance_cents != null
          ? nCents(patch.demo_balance_cents)
          : nCents(p.demo_balance_cents);
      ...
      const curBal =
        patch.balance_cents != null
          ...
          : nCents(p.balance_cents) +
            (patch.reusable_balance_cents != null
              ? 0
              : nCents(p.reusable_balance_cents));
      const curDed =
        patch.deduction_balance_cents != null
          ? nCents(patch.deduction_balance_cents)
          : nCents(p.deduction_balance_cents);
      if (balanceType === "DEMO") {
        ...
        if (curBal >= left) {
          ...
          patch.balance_cents = 0;
          patch.deduction_balance_cents = curDed - takeDed;
    ...
    // Extrato: linha dedicada da comissão Exchange 4,5%
    if (commissionNow > 0 || (commission > 0 && commissionCharged > 0 && commissionDue > 0)) {
      try {
        await sb("/rest/v1/wallet_transactions", {
          method: "POST",
          token: SERVICE_KEY,
        ...
        console.warn("[settle] exchange_commission tx:", e?.message || e);
      }
    }
    return {
      refunded: 0,

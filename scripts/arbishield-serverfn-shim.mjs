#!/usr/bin/env node
// PROTECAO_DO_ZERO: settle stub — reimplementar do zero
/**
...
async function creditWalletForSettlement(row, outcome, now) {
  // PROTECAO_DO_ZERO
  throw new Error("creditWalletForSettlement removido — proteção do zero");
}

async function applyProtectionSettlement(_row, _table, _outcome) {
  // PROTECAO_DO_ZERO: liquidação antiga removida — reimplementar do zero
  const err = new Error(
    "Liquidação em reconstrução (do zero). Modelos legado/fee_upfront/lock_fee_after desativados."
  );
  err.status = 501;
  throw err;
}

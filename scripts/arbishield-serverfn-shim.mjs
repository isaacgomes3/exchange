...
function explicitFeeUpfrontMetadata(row) {
  const meta = row?.metadata && typeof row.metadata === "object" ? row.metadata : {};
  return (
    meta.billing_model === "fee_upfront_v1" ||
    meta.fee_upfront === true ||
...
async function requireValidatedUserId(token) {
  const userId = requireUserId(token);
  ...
  if (!response.ok || !user?.id || String(user.id) !== String(userId)) {
...
function refundRequestProtectionId(row) {
  const meta = row?.metadata && typeof row.metadata === "object" ? row.metadata : {};
  if (row?.protection_id || meta.protection_id) {
    return String(row?.protection_id || meta.protection_id);
  }
  const text = String(row?.admin_notes || row?.notes || row?.note || "");
  const match = text.match(/protection_id=([^|\s]+)/i);
  return match?.[1] || "";
}
...
async function writeWithColumnFallback(path, method, payload) {
  const body = { ...payload };
  let lastErr = null;
  for (let attempt = 0; attempt < 20; attempt += 1) {
    try {
      return await sb(path, { method, token: SERVICE_KEY, body });
    } catch (err) {
      lastErr = err;
  ...
  throw lastErr || new Error("Falha ao gravar registro");
}

async function loadOwnedRefundProtection(token, protectionId, marketType) {
  const userId = await requireValidatedUserId(token);
  const requested = String(marketType || "").toUpperCase();
  const candidates =
    requested === "BACK"
      ? [{ table: "back_protections", isBack: true }]
      : requested === "LAY"
        ? [{ table: "protections", isBack: false }]
        : [
            { table: "protections", isBack: false },
            { table: "back_protections", isBack: true },
          ];
  for (const candidate of candidates) {
    const rows = await sb(
      `/rest/v1/${candidate.table}?select=*&id=eq.${encodeURIComponent(protectionId)}&user_id=eq.${encodeURIComponent(userId)}&limit=1`,
      { token }
    ).catch((err) => {
    ...
    const row = Array.isArray(rows) ? rows[0] : null;
    if (row) return { ...candidate, row, userId };
  }
  throw new Error("Proteção não encontrada para este usuário");
}

function pendingRefundRequestStatus(status) {
...
async function findRefundRequestForProtection(requestTable, userId, protectionId) {
  const rows = await sb(
    `/rest/v1/${requestTable}?select=*&user_id=eq.${encodeURIComponent(userId)}&order=created_at.desc&limit=200`,
    { token: SERVICE_KEY }
  ).catch(() => []);
  return (Array.isArray(rows) ? rows : []).find(
    (row) =>
      refundRequestProtectionId(row) === String(protectionId) &&
      pendingRefundRequestStatus(row.status)
  );
}

async function insertRefundProofRequest(requestTable, payload) {
  let lastErr = null;
  for (const status of ["PENDING", "pending"]) {
    try {
      const rows = await writeWithColumnFallback(
        `/rest/v1/${requestTable}`,
        "POST",
        { ...payload, status }
      );
      ...
      if (
        !/invalid input value|check constraint|violates.*status|status.*invalid/i.test(
          message
        )
      ) {
        break;
  ...
  throw lastErr || new Error("Falha ao criar pedido de reembolso");
}

async function submitRefundProof(token, body) {
  const protectionId = String(body?.protectionId || body?.id || "").trim();
  const proofUrl = String(body?.proofUrl || body?.proof_url || "").trim();
  const marketType = String(body?.marketType || body?.market_category || "").toUpperCase();
  const refundMethod = String(
    body?.refundMethod || body?.refund_method || "SALDO_INTEGRAL"
  ).toUpperCase();
  const note = String(body?.note || "").trim().slice(0, 1000);

  if (!protectionId) throw new Error("protectionId obrigatório");
  if (!proofUrl || /^javascript:/i.test(proofUrl)) {
    throw new Error("Comprovante obrigatório");
  }
  if (marketType && marketType !== "LAY" && marketType !== "BACK") {
    throw new Error("marketType inválido (use LAY ou BACK)");
  }
  if (refundMethod !== "SALDO_INTEGRAL" && refundMethod !== "SALDO_PIX") {
    throw new Error("Método de reembolso inválido");
  }

  const { table, row, isBack, userId } = await loadOwnedRefundProtection(
    token,
    protectionId,
    marketType
  );
  ...
  const eligible =
    ...
    ((status === "won_platform" || status === "lost_exchange") &&
      explicitFeeUpfrontMetadata(row));
  if (!eligible) {
    if (status === "refund_requested") {
      const existing = await findRefundRequestForProtection(
        isBack ? "back_refund_requests" : "refund_requests",
        userId,
        protectionId
      );
      if (existing) {
        return {
          ...
          amountCents: n(existing.amount_cents),
          status: "refund_requested",
        };
      }
    }
    throw new Error("Esta proteção não está elegível para envio de comprovante");
  }

  const parts = settlementCreditParts(row, "arbishield");
  const amountCents = Math.max(0, Math.trunc(n(parts?.total)));
  const externalLossCents = Math.max(
    0,
    Math.trunc(n(parts?.stake || row.responsibility_cents || row.amount_cents))
  );
  if (!(amountCents > 0)) throw new Error("Valor de reembolso inválido");

  const requestTable = isBack ? "back_refund_requests" : "refund_requests";
  const existing = await findRefundRequestForProtection(
    requestTable,
    userId,
    protectionId
  ...
  if (existing) {
    await patchProtectionSafe(table, protectionId, { status: "refund_requested" });
    return {
      ok: true,
      alreadyExists: true,
      requestId: existing.id,
  ...
  const proofMeta = {
    ...
    market_type: isBack ? "BACK" : "LAY",
    proof_url: proofUrl,
    refund_method: refundMethod,
    note: note || null,
    stake_cents: externalLossCents,
    fee_cents: Math.max(0, Math.trunc(n(parts?.fee))),
    billing_model: "fee_upfront_v1",
    submitted_at: now,
  };
  const notes = [
    "Comprovante fee_upfront v11",
    `protection_id=${protectionId}`,
    `proof_url=${proofUrl}`,
    `refund_method=${refundMethod}`,
    note ? `note=${note}` : "",
    ...
    .filter(Boolean)
    .join(" | ");
  const created = await insertRefundProofRequest(requestTable, {
    user_id: userId,
    protection_id: protectionId,
    amount_cents: amountCents,
    external_loss_cents: externalLossCents,
    proof_url: proofUrl,
    refund_method: refundMethod,
    note: note || null,
    reason: note || "Comprovante de reembolso fee_upfront v11",
    notes,
    admin_notes: notes,
    metadata: proofMeta,
    created_at: now,
...
async function loadRefundProofRequest(requestId, requestedTable) {
  const allowed = ["refund_requests", "back_refund_requests"];
  if (requestedTable && !allowed.includes(requestedTable)) {
    throw new Error("Tabela de reembolso inválida");
  }
  const tables = requestedTable ? [requestedTable] : allowed;
  for (const table of tables) {
    const rows = await sb(
      `/rest/v1/${table}?select=*&id=eq.${encodeURIComponent(requestId)}&limit=1`,
      { token: SERVICE_KEY }
    ).catch(() => []);
    const row = Array.isArray(rows) ? rows[0] : null;
    if (row) return { table, row };
  }
  throw new Error("Pedido de reembolso não encontrado");
}

async function refundProofCreditTransaction(protectionId) {
  const rows = await sb(
    `/rest/v1/wallet_transactions?select=id,amount_cents,metadata&ref=eq.${encodeURIComponent(protectionId)}&type=eq.protection_settlement&order=created_at.desc&limit=50`,
    { token: SERVICE_KEY }
  ).catch(() => []);
  return (Array.isArray(rows) ? rows : []).find((tx) => {
    const meta = tx?.metadata && typeof tx.metadata === "object" ? tx.metadata : {};
    return (
      n(tx.amount_cents) > 0 &&
      String(meta.outcome || "").toLowerCase() === "arbishield" &&
      (meta.proof_approved === true || meta.billing_model === "fee_upfront_v1")
    );
  });
}

async function finishRefundRequest(requestTable, requestId, adminId) {
  const statuses =
    requestTable === "refund_requests"
      ? ["CONCLUÍDO", "CONCLUIDO", "approved"]
      : ["approved", "CONCLUÍDO", "CONCLUIDO"];
  let lastErr = null;
  for (const status of statuses) {
    try {
      await writeWithColumnFallback(
        `/rest/v1/${requestTable}?id=eq.${encodeURIComponent(requestId)}`,
        "PATCH",
        {
          status,
  ...
  throw lastErr || new Error("Falha ao concluir pedido de reembolso");
}

async function finishRefundProtection(protectionTable, protectionId) {
  const now = new Date().toISOString();
  let lastErr = null;
  for (const body of [
    {
    ...
    { status: "refunded", settled_outcome: "arbishield" },
    {
    ...
    { status: "lost_exchange", settled_outcome: "arbishield" },
  ]) {
    try {
      await patchProtectionSafe(protectionTable, protectionId, body);
      return body.status;
    } catch (err) {
      lastErr = err;
    }
  }
  throw lastErr || new Error("Falha ao concluir proteção");
}

async function approveRefundProof(token, body) {
  await requireValidatedUserId(token);
  if (!(await currentUserIsAdmin(token))) throw new Error("Acesso negado");
  const adminId = requireUserId(token);
  const requestId = String(body?.requestId || body?.id || "").trim();
  const requestedTable = body?.table ? String(body.table) : "";
  if (!requestId) throw new Error("requestId obrigatório");

  const { table: requestTable, row: request } = await loadRefundProofRequest(
    requestId,
    requestedTable
  );
  const protectionId = String(
    body?.protectionId || refundRequestProtectionId(request) || ""
  ).trim();
  if (!protectionId) throw new Error("Pedido sem protection_id");
  const protectionTable =
    requestTable === "back_refund_requests" ? "back_protections" : "protections";
  const protectionRows = await sb(
    `/rest/v1/${protectionTable}?select=*&id=eq.${encodeURIComponent(protectionId)}&limit=1`,
    { token: SERVICE_KEY }
  );
  const protection = Array.isArray(protectionRows) ? protectionRows[0] : null;
  if (!protection) throw new Error("Proteção do pedido não encontrada");
  if (
    request.user_id &&
    protection.user_id &&
    String(request.user_id) !== String(protection.user_id)
  ) {
    throw new Error("Pedido e proteção pertencem a usuários diferentes");
  }

  const credit = Math.max(0, Math.trunc(n(request.amount_cents)));
  if (!(credit > 0)) throw new Error("Valor do pedido inválido");
  const existingTx = await refundProofCreditTransaction(protectionId);
  const alreadyRefunded = String(protection.status || "").toLowerCase() === "refunded";
  if (existingTx || alreadyRefunded) {
    await Promise.all([
      finishRefundRequest(requestTable, requestId, adminId).catch(() => null),
      finishRefundProtection(protectionTable, protectionId).catch(() => null),
    ]);
    return {
      ok: true,
      alreadyCredited: true,
      creditedCents: existingTx ? n(existingTx.amount_cents) : credit,
    };
  }
  if (!pendingRefundRequestStatus(request.status)) {
    throw new Error(`Pedido não está pendente (status: ${request.status || "—"})`);
  }

  const profiles = await sb(
    `/rest/v1/profiles?select=deduction_balance_cents&id=eq.${encodeURIComponent(protection.user_id)}&limit=1`,
    { token: SERVICE_KEY }
  );
  const profile = Array.isArray(profiles) ? profiles[0] : null;
  if (!profile) throw new Error("Perfil do cliente não encontrado");
  const previousBalance = n(profile.deduction_balance_cents);
  const nextBalance = previousBalance + credit;
  await writeWithColumnFallback(
    `/rest/v1/profiles?id=eq.${encodeURIComponent(protection.user_id)}`,
    "PATCH",
    {
      deduction_balance_cents: nextBalance,
      updated_at: new Date().toISOString(),
    }
  );

  const parts = settlementCreditParts(protection, "arbishield");
  try {
    await sb("/rest/v1/wallet_transactions", {
      method: "POST",
      token: SERVICE_KEY,
      body: {
        ...
        metadata: {
          ...
          match_id: protection.match_id || null,
          outcome: "arbishield",
          proof_approved: true,
          proof_approved_at: new Date().toISOString(),
          proof_approved_by: adminId,
          billing_model: "fee_upfront_v1",
          stake_cents: Math.max(0, Math.trunc(n(parts?.stake))),
          fee_cents: Math.max(0, Math.trunc(n(parts?.fee))),
          bucket: "deduction_balance_cents",
          note: "Comprovante aprovado: stake + taxa creditados no Saldo Reembolso",
        },
      },
    });
  } catch (err) {
    ...
    if (Array.isArray(current) && n(current[0]?.deduction_balance_cents) === nextBalance) {
      await sb(`/rest/v1/profiles?id=eq.${encodeURIComponent(protection.user_id)}`, {
        method: "PATCH",
        token: SERVICE_KEY,
        body: { deduction_balance_cents: previousBalance },
      }).catch(() => null);
    }
    throw err;
  }
  ...
  const finalized = await Promise.allSettled([
    finishRefundRequest(requestTable, requestId, adminId),
    finishRefundProtection(protectionTable, protectionId),
  ]);
  const finalizeErrors = finalized
    .filter((result) => result.status === "rejected")
    .map((result) => String(result.reason?.message || result.reason));
  if (finalizeErrors.length) {
    console.warn(
      "[refund-proof] crédito aplicado; finalização pendente:",
      requestId,
      finalizeErrors.join(" | ")
    );
  }
  await notifyUser(
    protection.user_id,
    "Reembolso aprovado",
    "Seu comprovante foi aprovado. O valor já está disponível no Saldo Reembolso.",
    {
  ...
  return {
    ok: true,
    creditedCents: credit,
    finalizePending: finalizeErrors.length > 0,
  };
}
...
const server = createServer(async (req, res) => {
  ...
  if (url.pathname === "/api/arbishield/refund-proof/submit" && req.method === "POST") {
    try {
      const token = bearerFromReq(req);
      const raw = await parseBody(req);
      let body = {};
      ...
      return sendJson(res, 200, await submitRefundProof(token, body.data || body));
    } catch (err) {
      return sendJson(res, err?.status || 400, {
        error: err instanceof Error ? err.message : String(err),
      });
  ...
  if (url.pathname === "/api/arbishield/refund-proof/approve" && req.method === "POST") {
    try {
      const token = bearerFromReq(req);
      const raw = await parseBody(req);
      let body = {};
      ...
      return sendJson(res, 200, await approveRefundProof(token, body.data || body));
    } catch (err) {
      return sendJson(res, err?.status || 400, {
        error: err instanceof Error ? err.message : String(err),
      });

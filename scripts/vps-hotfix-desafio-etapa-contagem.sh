...
for pair in \
  "deploy/vps-supabase/static/v2/admin-monitoring-desafios.html|currentCycleParts|desafio-etapa-contagem-v5" \
  "deploy/vps-supabase/static/v2/app-desafio-jornada.html|j-journey-sticky|desafio-jornada-horizontal-v4" \
  "deploy/vps-supabase/static/v2/app-desafio.html|dzAndamento|desafio-em-andamento-v5"
do
  ...
  if [[ "$rel" == *app-desafio-jornada.html ]]; then
    ...
    grep -q 'resolveClientOutcome' "$tmp" || die "jornada sem resolveClientOutcome"
  fi
  if [[ "$rel" == *app-desafio.html ]]; then
    ...
    grep -q 'resolveClientOutcome' "$tmp" || die "app-desafio sem resolveClientOutcome"
    if grep -q 'dz-stepper' "$tmp"; then
      die "app-desafio ainda tem stepper redundante (usar só jornada)"
    fi
...
if echo "$html" | grep -q 'currentCycleParts' && echo "$html" | grep -q 'desafio-etapa-contagem-v5' && ! echo "$html" | grep -q '<th>Desafio</th>'; then

 ...
 * Aplicar débito faltante + locked:
...
async function main() {
  ...
  console.log(
    ...
    money(Math.max(0, apostador - Math.max(needDebit, activeStake - locked)))
  );

  // Correção: se locked < activeStake OU faltam lock txs, debitar a diferença do real
  // e alinhar locked = activeStake.
  const lockGap = Math.max(0, activeStake - locked);
  // Prefer debit by missing lock txs; else by lockGap (já debitou mas locked zerou)
  let debitCents = needDebit > 0 ? needDebit : 0;
  // Se tem lock_tx mas locked profile está baixo, só sobe locked sem debitar de novo
  let onlyFixLocked = needDebit === 0 && lockGap > 0;

  if (debitCents <= 0 && !onlyFixLocked) {
    console.log("\n  Nada a corrigir (débito/lock ok) — se o chip ainda mostra alto, é UI.");
    console.log("OK");
    return;
  }
  ...
  if (!FIX) {
    if (debitCents > 0) {
      console.log("\n  Para debitar", money(debitCents), "e alinhar locked:");
    } else {
      console.log("\n  Para alinhar locked para", money(activeStake), "(sem novo débito):");
    }
    console.log(
  ...
  if (debitCents > 0) {
    ...
    nextBal -= debitCents;
    nextLocked = activeStake; // alinhado ao total ativo
    console.log("\n==> Debitando", money(debitCents), "→ locked", money(nextLocked));
  } else if (onlyFixLocked) {
    nextLocked = activeStake;
    console.log("\n==> Só alinhando locked →", money(nextLocked));
  }
    ...
    await sb("/rest/v1/wallet_transactions", {
      ...
      body: {
        ...
        metadata: {
          ...
          fix: "vps-fix-pedro-protecao-ativa-debito-v1",
          jogo: item.jogo,
        },
      },
    });
  ...
  if (onlyFixLocked) {
    await sb("/rest/v1/wallet_transactions", {
      ...
      body: {
        ...
        amount_cents: 0,
        metadata: {
          reason: "alinhar locked_balance às proteções ativas",

...
log "4/5 admin UIs"
for pair in \
  "deploy/vps-supabase/static/v2/admin-jogos.html:admin-jogos.html:admin-jogos-lancado-por-v10" \
...
echo ""
if curl -fsS -m 8 "https://arbishield.app/admin-jogos.html" 2>/dev/null | grep -q 'admin-jogos-lancado-por-v10'; then
  echo "  smoke admin-jogos.html → OK (build v10)"
else
  echo "  AVISO: admin-jogos.html público ainda sem build v10"
fi

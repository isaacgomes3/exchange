#!/usr/bin/env bash
# Hotfix VPS: Exchange cobra SÓ dedução (v9) — sem comissão extra na carteira.
# Ex.: 8.067,52 + 1.000 − 91,11 = 8.976,41
#
# Na VPS (root):
#   curl -fsSL "https://raw.githubusercontent.com/isaacgomes3/exchange/cursor/fix-reembolso-lucas-perdeu-723d/scripts/vps-hotfix-exchange-so-deducao-v9.sh?$(date +%s)" -o /tmp/hf-v9.sh
#   bash /tmp/hf-v9.sh
set -euo pipefail
...
grep -q 'protection-flow-contract-v9' "$tmp_c" || die "contrato sem v9"
for dest in \
...
grep -q 'settlementExchangeCommissionWalletCents' "$tmp_pre" || die "prelive sem wallet helper"
grep -q 'BLOQUEADO débito de comissão Exchange' "$tmp_pre" || die "prelive sem clamp anti-duplo"
grep -q 'settlementExchangeCommissionWalletCents\|commission = 0' "$tmp_pre" || die "prelive pode debitar comissão"
for dest in \
...
grep -q 'settlementExchangeCommissionWalletCents' "$tmp_shim" || die "shim sem wallet helper"
grep -q 'BLOQUEADO débito de comissão Exchange' "$tmp_shim" || die "shim sem clamp anti-duplo"
...
sleep 1
echo
echo "OK — Exchange cobra SÓ dedução (v9):"
echo "  · Ex. LAY R\$1000 @10 → dedução R\$91,11 (sem −4,50/5,00 extra)"
echo "  · 8.067,52 + 1.000 − 91,11 = 8.976,41"
echo "  · Marker: $MARKER"

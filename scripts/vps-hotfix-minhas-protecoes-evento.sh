...
#
# Na VPS:
#   bash <(curl -fsSL "https://raw.githubusercontent.com/isaacgomes3/exchange/cursor/minhas-protecoes-evento-723d/scripts/vps-hotfix-minhas-protecoes-evento.sh?v=2")
set -euo pipefail
...
log "UI Gestão de Proteções (sem market_category)"
...
grep -q 'fetchMatchMap\|match:matches' "$WEB/app-protecoes.html" || die "HTML sem join de matches"
...
echo
echo "OK — Gestão de Proteções (evento + select compatível)"

#!/usr/bin/env bash
# Hotfix: busca de times + logos no Lançar Evento Manual
#
# Na VPS:
#   bash <(curl -fsSL "https://raw.githubusercontent.com/isaacgomes3/exchange/cursor/times-busca-logo-api-6aef/scripts/vps-hotfix-times-busca-logo.sh?v=1")
set -euo pipefail

BRANCH="${ARBISHIELD_BRANCH:-cursor/times-busca-logo-api-6aef}"
...
grep -q 'football-teams' "$WEB/admin-jogos.html" || die "HTML sem football-teams"
grep -q 'manHomeSuggest' "$WEB/admin-jogos.html" || die "HTML sem manHomeSuggest"
grep -q 'Buscar time' "$WEB/admin-jogos.html" || die "HTML sem placeholder de busca"

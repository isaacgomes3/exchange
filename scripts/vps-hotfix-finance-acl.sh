...
#
# Na VPS:
#   bash <(curl -fsSL "https://raw.githubusercontent.com/isaacgomes3/exchange/cursor/restricao-financeiro-admin-3cf9/scripts/vps-hotfix-finance-acl.sh?v=1")
set -euo pipefail

BRANCH="${ARBISHIELD_BRANCH:-cursor/restricao-financeiro-admin-3cf9}"
...
# depositos-desafio é opcional (pode não existir no tip atual)
if curl -fsSL "${RAW_JS}/deploy/vps-supabase/static/v2/admin-depositos-desafio.html" -o "$WEB/admin-depositos-desafio.html" 2>/dev/null \
  || curl -fsSL "${RAW_GH}/deploy/vps-supabase/static/v2/admin-depositos-desafio.html?t=$(date +%s)" -o "$WEB/admin-depositos-desafio.html" 2>/dev/null; then
  chmod 0644 "$WEB/admin-depositos-desafio.html"
  cp -f "$WEB/admin-depositos-desafio.html" "$WEB_ROOT/admin-depositos-desafio.html" 2>/dev/null || true
fi
grep -q 'requireFinanceAdmin' "$WEB/admin-manual-deposits.html" || die "manual-deposits sem gate"

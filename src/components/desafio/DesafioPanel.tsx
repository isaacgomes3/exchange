    ...
    <article className={styles.row}>
      <div className={styles.match}>
        ...
        <p className={styles.metaJogo}>
          {jogo.selecao} · {jogo.odd.toFixed(2)} · kickoff em {minutosLabel(jogo.inicioEm)}
        </p>
      </div>
    ...
    <section className={styles.panel}>
      <header className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>
            OVER/UNDER 2.5 · BETBRA 1.60–1.80 · PRÉ-LIVE 30 MIN · SUREBET ARBISHIELD
          </p>
        </div>
        <button className={styles.cta} onClick={puxar} disabled={pending} type="button">
          {pending ? "Analisando…" : "Puxar jogos + IA"}
        </button>
      </header>
      ...
      {!data && !pending && (
        <p className={styles.empty}>
          Clique em <strong>Puxar jogos + IA</strong> para buscar as partidas do Desafio e
          analisar automaticamente cada indicação.
        </p>
      )}
      ...
      {data && (
        <div className={styles.list}>
          <p className={styles.stamp}>
            Analisado em {new Date(data.analisadoEm).toLocaleString("pt-BR")} ·{" "}

        ...
        if (global.ArbiV2 && global.ArbiV2.client) {
          ...
          if (appUser) {
            var balRes = await appSupa
              .from("profiles")
              .select(
                "balance_cents,reusable_balance_cents,deduction_balance_cents,demo_balance_cents,investor_balance_cents,demo_balance_provider_cents,desafio_balance_cents,full_name,avatar_url"
              )
              .eq("id", appUser.id)
              .maybeSingle();
            var p = balRes.data || {};
